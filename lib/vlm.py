...
def pdf_analyze(pdf_path, vlm_model):
    """pdf_analyze
    pdf를 이미지로 파싱한 후 vlm모델을 이용하여 텍스트로 변환하는 함수입니다.
    ...
    output_txt_path = "./manual_layout.txt" # 전처리 결과를 저장할 텍스트 파일 경로

    image_paths = convert_pdf_to_images(pdf_path)
    print("2. [VLM] Qwen2-VL-7B-AWQ 모델 로드 중...")
    vlm_model = vlm_model.strip()  # 공백 제거
